...
*
* $Id: relay.c,v 1.19 2001/02/05 02:20:53 dfs Exp $
*
***********************************************************************/
...
#endif
    sendPacket(sh->interface->sessionSock, &packet, size);
}
		...
		int size)
{
	...
	sendPacket(Interfaces[i].discoverySock, packet, size);
    }
		...
		int size)
{
    ...
    sendPacket(Interfaces[ifIndex].discoverySock, packet, size);
}
	       ...
	       char const *errMsg)
{
    ...
    size = ntohs(packet.length) + HDR_SIZE;
    if (code == CODE_PADT) {
	sendPacket(iface->discoverySock, &packet, size);
    } else {
	sendPacket(iface->sessionSock, &packet, size);
    }
}
